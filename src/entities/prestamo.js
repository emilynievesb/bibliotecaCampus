...
class Prestamo {
  ...
  async obtenerPrestamosPorUsuario() {
    let sql = /*sql*/ `SELECT u.nombre AS nombreUsuario, u.apellido AS apellidoUsuario, l.titulo AS nombreLibro,
    p.fecha_prestamo AS fechaPrestamo, p.fecha_devolucion AS fechaDevolucion, p.estado AS estado
    FROM prestamo p
    INNER JOIN usuario u ON p.id_usuario = u.id_usuario
    INNER JOIN libro l ON p.id_libro = l.id_libro
    WHERE CONCAT(u.nombre, \" \", u.apellido) =\"${this.nombre_usuario}\"`;
    try {
      const result = await executeQuery(sql);
      return result.data;
    } catch (error) {
  ...
  async obtenerLibrosPrestadosPorUsuario() {
    let sql = /*sql*/ `SELECT l.titulo AS nombreLibro,
    p.fecha_prestamo AS fechaPrestamo, p.fecha_devolucion AS fechaDevolucion, p.estado AS estado
    FROM prestamo p
    INNER JOIN usuario u ON p.id_usuario = u.id_usuario
    INNER JOIN libro l ON p.id_libro = l.id_libro
    WHERE CONCAT(u.nombre, \" \", u.apellido) =\"${this.nombre_usuario}\" AND p.estado = "Prestado"`;
    try {
      const result = await executeQuery(sql);
      return result.data;
    } catch (error) {
